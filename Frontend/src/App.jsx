...
import {
  ...
  LogOut,
  AlertCircle,
  Users // NEW: Added Users icon for the sidebar
} from 'lucide-react'
...
import UserManagement from './pages/UserManagement' // NEW: Imported the new UserManagement page

const baseNavItems = [
  { path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { path: '/admin', label: 'Resources', icon: Settings, adminOnly: true },
  // NEW: Added Users link to the sidebar (Only visible to Admins)
        ...
        <main className={hideSidebar ? 'flex-1 min-h-screen' : 'ml-0 md:ml-[260px] flex-1 min-h-screen'}>
          ...
          <div className="pt-20"> {/* Add padding for the fixed navbar */}
            <Routes>
              ...
              />
              {/* NEW: Protected Route for User Management (Admins only) */}
              <Route
                  path="/users"
                  element={<ProtectedRoute element={<UserManagement />} user={user} allowedRoles={['ADMIN']} />}
              />
...
function App() {
  ...
  const renderContent = () => {
    ...
    if (isTerms) return <TermsConditions />;
    return <AppLayout />;
  };
